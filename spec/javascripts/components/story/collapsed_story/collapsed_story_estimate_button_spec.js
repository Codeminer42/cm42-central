...
describe('<CollapsedStoryEstimateButton />', () => {
  it("renders component with 'Fibonacci' point scale", () => {
    const props = { pointValues: ['1','2','3','5','8'] }
    const wrapper = shallow(<CollapsedStoryEstimateButton project={props}/>);
    const text = wrapper.text()

    props.pointValues.forEach((value)=>{
      expect(text).toContain(value)
    });
  });

  it("renders component with 'Powers of two' point scale", () => {
    const props = { pointValues: ['1','2','4','8'] }
    const wrapper = shallow(<CollapsedStoryEstimateButton project={props}/>);
    const text = wrapper.text()
  ...
  it("renders component with 'Linear' point scale", () => {
    const props = { pointValues: ['1','2','3','4','5'] }
    const wrapper = shallow(<CollapsedStoryEstimateButton project={props}/>);
    const text = wrapper.text()
