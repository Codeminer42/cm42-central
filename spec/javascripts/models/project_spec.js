...
describe('Project model', () => {
  ...
  describe('changesets', () => {
    it('should load changesets when last_changeset_id is changed', function () {
      ...
      const spy = sinon.spy(this.project, 'handleChangesets');
      const changesets = [{ changeset: { id: 2, story_id: 456, project_id: 789 } }];
    ...
    it('should reload changed stories from changesets', function () {
      const changesets = [{ changeset: { id: 123, story_id: 456, project_id: 789 } }];
      const getSpy = sinon.spy(this.project.stories, 'get');
      const fetchSpy = sinon.spy(this.story, 'fetch');
    ...
    it('should load new stories from changesets', function () {
      const story_json = { story: { id: 987, title: 'New changeset story' } };
      const server = sinon.fakeServer.create();
      ...
      const changesets = [{ changeset: { id: 123, story_id: 987, project_id: 789 } }];
      const getSpy = sinon.spy(this.project.stories, 'get');
      const addSpy = sinon.spy(this.project.stories, 'add');
      const initial_collection_length = this.project.stories.length;
  ...
  describe('iterations', () => {
    it('should get the right iteration number for a given date', function () {
      ...
      let compare_date = new Date('2011/07/25');
  ...
  describe('start date', () => {
    it('should return the start date', function () {
      ...
      const expected_date = new Date('2011/07/23');
      const fake_today = new Date('2011/07/29');
  ...
  describe('velocity', () => {
    ...
    describe('when velocity is not set', () => {
      describe('velocityIsFake', () => {
    ...
    describe('when velocity is set to 20', () => {
      beforeEach(function () {
    ...
    describe('when velocity is set to less than 1', () => {
      beforeEach(function () {
    ...
    describe('when velocity is set to the same as the real value', () => {
      describe('velocity', () => {
        beforeEach(function () {
  ...
  describe('appendIteration', () => {
    ...
    it('should add the first iteration to the array', function () {
      const stub = sinon.stub(Iteration, 'createMissingIterations');
      stub.returns([]);
    ...
    it('should create missing iterations if required', function () {
      const spy = sinon.spy(Iteration, 'createMissingIterations');
  ...
  describe('columns', () => {
    ...
    it('should return the columns after a given column', function () {
      ...
      expect(() => { project.columnsAfter('#foobar'); }).toThrow('#foobar is not a valid column');
    });
    ...
    it('should return the columns before a given column', function () {
      ...
      const project = this.project;
      expect(() => { project.columnsBefore('#foobar'); }).toThrow('#foobar is not a valid column');
    });
  });
